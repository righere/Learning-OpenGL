...
int main() 
{
	...
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "OpenGL Create texture", NULL, NULL);
	if (window == NULL) 
	{
	...
	{
		std::cout << "Failed load a texture" << std::endl;
	}
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		float setAlpha = 0.5;
		glUniform1f(glGetUniformLocation(shader.ID, "alpha"), setAlpha);
